...
class SnowToSwe:
    ...
    def __compactH(self, x, ts):
        # .d  -> today
        # .dd -> tomorrow
        age_d = 0 if x.iloc[0] == 0 else x.iloc[3]
        h_dd = x.iloc[0] / (1 + (x.iloc[2] * EARTH_GRAVITATIONAL_ACCELERATION * ts) / self.eta_null * exp(-self.k * x.iloc[1] / x.iloc[0]))
        h_dd = x.iloc[1] / self.rho_max if x.iloc[1] / h_dd > self.rho_max else h_dd
        h_dd = 0 if x.iloc[0] == 0 else h_dd
        swe_dd = x.iloc[1]
        age_dd = 0 if x.iloc[0] == 0 else age_d + 1
        rho_dd = 0 if x.iloc[0] == 0 else swe_dd / h_dd
