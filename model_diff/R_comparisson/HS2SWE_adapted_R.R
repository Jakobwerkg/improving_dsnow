...
#
# returns:
#   a list (one element per input column). Each element contains the MR objects
#   for each continuous snow episode in that column.
#
# CHANGES (minimal, marked with "<<< CHANGED"):
# 1) Function returns MR (instead of SWE matrix).
# 2) MR includes MR$SWE (matrix) holding SWE time series (same columns as MR$HS etc.).
# 3) Whenever a new layer is added (rbind on HS/RHO/OVB/AGE), MR$SWE is also rbinded,
#    so dimensions stay consistent.
...
hs2swe <- function(
  ...
  MR_out <- lapply(1:ncol(idata), function(i) {
    x <- idata[, i]
    ...
    if (length(psdix) > 0) {
      ...
      MR_list <- vector("list", length(sepix) - 1)  # <<< CHANGED: collect MR per episode
      ...
      for (tsrix in 1:(length(sepix) - 1)) {
        ...
        MR <- list(
          ...
          SWE = matrix(0, nrow = 1, ncol = length(HS))  # <<< CHANGED: add SWE container
        )
        ...
        for (tn in 2:length(HS)) {
          ...
          if (HS[tn] > sum(MR$HS[, tn]) & tn == 2) {
            ...
            MR$SWE <- rbind(MR$SWE, matrix(0, nrow = 1, ncol = length(HS))) # <<< CHANGED: keep dims
        ...
        # compute SWE and store it in MR$SWE (row 1 holds total SWE)
        SWE <- colSums(MR$HS * MR$RHO / 100)
        MR$SWE[1, tn] <- SWE[tn]  # <<< CHANGED: store SWE at current timestep
      }

      # store MR for this episode (also store original indices)
      MR_list[[tsrix]] <- list(
        index = psdix[(sepix[tsrix] + 1):sepix[tsrix + 1]],
        MR = MR
      )
  ...
  # <<< CHANGED: return MR only (one entry per input column)
  if (!is.null(colnames(idata))) {
    names(MR_out) <- colnames(idata)
  }
  return(MR_out)
}