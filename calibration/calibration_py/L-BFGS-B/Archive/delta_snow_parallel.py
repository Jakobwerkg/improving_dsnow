...
def evaluate_model_parallel(df_all, r_script_path, params_unscaled, workers, temp_dir="/tmp"):
    ...
    merged = df_all.merge(out.rename(columns={"station": "name"}),
                          on=["date", "name"], how="inner")
...
def main():
    ...
    # Logging directory
    output_dir = Path("calibration/calibration_py/python_calib_log")
