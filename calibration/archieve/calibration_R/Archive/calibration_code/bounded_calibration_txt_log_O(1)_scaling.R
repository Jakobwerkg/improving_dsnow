#-------------------------------------------------------------------------
# Parameter optimization for ΔSNOW (1SNOW) using alt-winter split only
# Target: minimize RMSE between modeled SWE (ΔSNOW) and observed SWE.
#
# Reliability features:
# - Single split mode (alternating winters): even-index winters -> FIT, odd -> VAL
# - No duplicate-timestamp processing (as requested); time series are only sorted
# - Robust column mapping (Hobs/SWEobs naming variants)
# - Parallelization with safe worker setup (no %do% on workers; packages preloaded)
# - Objective uses a large finite penalty instead of Inf to keep optimizers alive
# - Graceful fallback if all optim methods fail (preserves log structure/paths)
#
# Parameter handling:
# - UNSCALED parameter guess + bounds in physical units
# - Improved scaling so all scaled parameters are ~O(1) for good conditioning
# - Tighter, literature-consistent bounds (esp. rho.null, rho.max, k.ov, tau, eta.null)
#
# Logging:
# - Produces the same TXT log structure and filename pattern as your original script
#
# Author notes:
# - Base code: H. Schellander, 08/2025
# - Reliability/documentation upgrade
#-------------------------------------------------------------------------
...
calib_comment <- "dsnow_calib_Win21_Mag25_alt_winters_bounded_thight_param_space_experimental"

# Hydrological season window (Aug 1 .. Jul 31)
season.start <- "-08-01"
season.end   <- "-07-31"

# Optional cutoff at start of a hydrological season; set to NA to disable
# Example: cutoff.year <- 2020 keeps data >= "2020-08-01"
cutoff.year <- NA_integer_

# --- Data loading & station filtering --------------------------------------
rda_path <- "/Users/jakobwerkgarner/code/Master_Delta/calibration_data/exports/d_obs.rda"
d_obs    <- get(load(rda_path))  # list(name -> zoo), columns include depth + SWE

# Normalize station names (trim, collapse spaces)
clean_names <- function(x) trimws(gsub("\\s+", " ", x))
names(d_obs) <- clean_names(names(d_obs))

# Remove stations (duplicates, typos, outliers) — same list as before
...
# Build valid winters for a station (as list of zoo objects).
# "Valid" => >= 200 rows; if <365 rows, reject if first/last Hobs > edge_thresh (when Hobs exists).
...
# Alternating winters: even index -> fit, odd -> val
build_splits_alt <- function(d_obs, season.start = "-08-01", season.end = "-07-31") {
  ...
  for (n in stations) {
    d <- d_obs[[n]]
    d <- d[order(index(d)), ]  # per request: no duplicate handling; just sort
...
# Optional: exclude Sta.Maria from the fit set (analysis-only)
d_obs_fit[["Sta.Maria"]] <- NULL

# --- Hydrological blocks ----------------------------------------------------
start_of_block <- 8  # August

# Map calendar dates to hydrological season year (e.g., 2021-08-01..2022-07-31 -> season 2021)
...
# Look for plausible H (snow depth) and SWE columns in case of naming variants.
...
# Build calibration tibble from the split FIT set
# Assumptions: hs in centimeters -> converted to meters; SWE in mm or kg/m^2 (equivalent)
build_fit_tibble <- function(d_obs_fit, start_of_block = 8) {
  out <- lapply(names(d_obs_fit), function(nm) {
    ...
    hs_raw  <- cd[[colmap$h_col]]    # cm -> convert to m
    swe_raw <- cd[[colmap$swe_col]]  # mm or kg/m^2 (treated equivalent)
...
# --- Parameter setup (UNSCALED guess, improved scaling, tighter bounds) ----
# ΔSNOW (1SNOW) — Parameter Summary and Physical Context
# -------------------------------------------------------
# The ΔSNOW (1SNOW) model is a semi-empirical snow compaction model
# requiring the calibration of seven physical parameters. Each parameter
# represents a measurable or physically interpretable quantity. The chosen
# calibration bounds are based on literature values (Martinec, 1977; Sturm
# & Holmgren, 1998; Vionnet et al., 2012; SNTHERM; Crocus), constrained
# to physically realistic snowpack conditions.

# Parameter names and physical interpretations:
#
#  rho.max   (kg/m^3)
#    → Maximum density a snow layer can attain.
#      Defines the asymptotic densification limit for aged or compacted snow.
#      Literature: typically 300–600 kg m⁻³ (seasonal snow rarely exceeds 500 kg m⁻³).
#      Used range: 350–550 kg m⁻³.
#      Optimized values near 400 kg m⁻³ match observed end-of-season bulk densities.
#
#  rho.null  (kg/m^3)
#    → New-snow density ρ₀: density of a freshly fallen layer.
#      Strongly temperature- and wind-dependent; typical 70–110 kg m⁻³,
#      broader observed range 10–350 kg m⁻³.
#      Used range: 70–120 kg m⁻³.
#      Lower values (≈80 kg m⁻³) correspond to light Alpine snowfall.
#
#  c.ov      (Pa^-1)
#    → Overburden factor: controls additional ("unsteady") compaction
#      from overlying load. Physically limited by the overburden stress σ₀
#      (~1000 Pa). To keep strain ε < 1, cov ≤ 10⁻³ Pa⁻¹ is imposed.
#      Used range: 0–1×10⁻³ Pa⁻¹.
#      Typical optimum ≈5×10⁻⁴ Pa⁻¹.
#
#  k.ov      (–)
#    → Overburden exponent: dimensionless exponent scaling how density
#      influences overburden strain. Must remain positive and real.
#      Used range: 0.05–2.0 (tighter than generic 0.01–10 for stability).
#
#  k.exp     (m^3 kg^-1)
#    → Density-dependence exponent "k" in the exponential law of compaction.
#      Determines how densification rate depends on ρ; governs viscosity evolution.
#      Literature: 0.011–0.08 m³ kg⁻¹ (SNTHERM ≈ 0.021, Crocus ≈ 0.023,
#      Alpine-new snow ≈ 0.185). Used range: 0.01–0.08.
#      Optimum around 0.03 m³ kg⁻¹.
#
#  tau       (m)
#    → Threshold deviation: buffer against small snow-depth or mass
#      fluctuations (e.g., observation error). Expected magnitude a few cm.
#      Used range: 0.01–0.05 m (= 1–5 cm). Low sensitivity within this range.
#      Optimum around 0.024 m (≈ 2.4 cm).
#
#  eta.null  (Pa s)
#    → Newtonian viscosity at zero density (η₀): controls the rate of
#      compaction in the viscous densification term.
#      Literature: 1×10⁶–2×10⁷ Pa s; Sturm & Holmgren (1998) ≈ 8.5×10⁶ Pa s.
#      Used range: 3×10⁶–2×10⁷ Pa s.
#
# Scaling:
#   Parameters differ by orders of magnitude (10⁻⁴ → 10⁷), so each is
#   rescaled to order unity to improve numerical conditioning in optimization.
#   The “par_scale” vector below defines this transformation.
#
# ---------------------------------------------------------------------------


# UNSCALED initial guess (in physical units)
par_guess_unscaled <- c(
  ...
  k.ov      = 0.227786,
  k.exp     = 0.0290256,
  tau       = 0.02489556,     # meters
  eta.null  = 8.792253e6
)

# Improved scaling (aim for O(1) in the optimizer)
par_scale <- c(
  rho.max   = 500,    # 350–550 -> scaled ~0.7–1.1
  rho.null  = 100,    # 90–120  - scaled ~0.9–1.2 (ρ0 nicely O(1))
  c.ov      = 1e-3,   # 0–1e-3  -> scaled 0–1
  k.ov      = 1,      # 0.05–2  -> scaled ~0.05–2
  k.exp     = 0.05,   # 0.01–0.08 -> scaled ~0.2–1.6
  tau       = 0.05,   # 0.01–0.05 m -> scaled 0.2–1
  eta.null  = 1e6     # 3e6–2e7 -> scaled ~0.3–2
)

# Tighter UNSCALED bounds (literature-consistent; improves stability)
lower_unscaled <- c(
  rho.max   = 400,
  rho.null  =  100,
  c.ov      =   0,
...
# Helper: validate names and scale vectors consistently
...
# --- Objective (RMSE) with finite penalty for pathological steps ------------
.big_penalty <- 1e12  # large finite value to avoid Inf in the optimizer

# NOTE: 'par' arrives scaled; multiply by 'scale' inside.
minimize_score <- function(data, par, scale, verbose = FALSE) {
  ...
  ll <- foreach(
    ...
    l <- lapply(yrs, function(y) {
      ...
      # Create contiguous daily date sequence, right-join observations
      right  <- data.frame(date = seq(min(left$date), max(left$date), by = "1 day"))
      joined <- dplyr::right_join(left, right, by = "date")
      
      # Model SWE with ΔSNOW (nixmass::swe.delta.snow)
      swe_mod <- tryCatch({
        joined |>
...
# --- Metrics helper (used for logging & fallback) ---------------------------
...
# Ensure workers have required packages loaded (no %do% on workers)
...
# Synthesize a fallback row if optimizer errored or produced unusable results
...
saveRDS(opt_df, file = "opt_results_orig.rds")  # filename unchanged

# --- Logging (exact same structure/labels/order as original) ----------------
best_idx <- which.min(opt_df$value)

# Robust extraction of parameter vector, even if columns are unusual
...
# Evaluate metrics at the best (or fallback) parameters for the log
